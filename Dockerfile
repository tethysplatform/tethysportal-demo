FROM tethysplatform/tethys-core:4.5.1-py3.13-dj4.2
...
ENV OPEN_SKY_CLIENT_ID="tethys_demo_portal-api-client"
ENV OPEN_SKY_CLIENT_SECRET="sTRth6x9f9lhJ2JYraxtaMWjuxM0n47j"
...
RUN pip install --no-cache-dir -r app_requirements/pip_package_requirements.txt
...
RUN cd ${TETHYS_HOME}/apps/tethysapp-flight_tracker/tethysapp-flight_tracker && tethys install -w -N -q && \
    ...
    cd ${TETHYS_HOME}/apps/tethysapp-usgs_mrms/tethysapp-usgs_mrms && tethys install -w -N -q && \
    cd ${TETHYS_HOME}/apps/tethysapp-Hydro_sync && tethys install -w -N -q
...
RUN cd ${TETHYS_HOME}/tethysdash_plugins/tethysdash_plugin_cnrfc && pip install . && \
    ...
    cd ${TETHYS_HOME}/tethysdash_plugins/tgf_tethysdash_plugins && pip install .

RUN mkdir -p -m 777 ${TETHYS_PERSIST}/data/tethysdash

RUN chmod -R 777 /opt/conda/envs/tethys/lib/python3.13/site-packages/tethysdash_plugin_geoglows/
